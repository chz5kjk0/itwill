...
<body>
	<%
	// 세션 객체에 저장된 세션 아이디(속성명 "sId") 가져와서 변수 id 에 저장
	String id = (String)session.getAttribute("sId");
	%>
	<header>
		<!-- 세션 아이디 존재 여부에 따라 다른 링크 표시 -->
		<!-- 세션 아이디가 없을 경우 로그인 & 회원가입 링크 표시 -->
		<!-- 세션 아이디가 있을 경우 아이디 & 로그아웃 링크 표시 -->
		<!-- 단 자신의 아이디 클릭 시 select2_detail.jsp 페이지로 이동(파라미터 : 아이디) -->
		<%if(id == null || id.equals("")) { %>
			<h5><a href="loginForm2.jsp">로그인</a> | <a href="insertForm2.jsp">회원가입</a></h5>
		<%} else { %>
			<!-- 또한 로그인 된 아이디(= 세션 아이디)가 "admin" 이면 관리자 페이지 링크 추가 -->
			<h5>
				<a href="select2_detail.jsp?id=<%=id %>"><%=id %> 님</a> | <a href="logout.jsp">로그아웃</a> 
				<%if(id.equals("admin")) {%>
